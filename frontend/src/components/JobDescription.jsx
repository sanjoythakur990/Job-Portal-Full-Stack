  ...
  return (
    <div className="max-w-7xl mx-auto my-10">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={isApplied ? null : applyJobHandler} disabled={isApplied} className={`rounded-lg ${isApplied ? 'bg-gray-600 cursor-not-allowed' : 'bg-[#6A38C2] hover:bg-[#5f32ad]'}`}>{isApplied ? 'Already applied' : 'Apply Now'}</Button>
      </div>
      ...
      <div className="my-5">
        ...
        <h1 className="font-bold my-1">Decription: <span className="pl-4 text-gray-800 font-normal">{singleJob?.description}</span></h1>
        <h1 className="font-bold my-1">Experience: <span className="pl-4 text-gray-800 font-normal">{singleJob?.experienceLevel} yrs</span></h1>
        ...
        <h1 className="font-bold my-1">Date Posted: <span className="pl-4 text-gray-800 font-normal">{singleJob?.createdAt.split("T")[0]}</span></h1>
      </div>
    </div>
  );
}
